...
def emulator_dyn_load(rpc_client, smu, voltage_V, currents_A):
    ...
    for current_A in currents_A[1:]:
        ...
        click.confirm(f"Confirm to switch to V = {voltage_V} V, A = {current_lim * 1000} mA", default=True)
        smu.source.leveli = - current_lim  # negative current, because smu acts as a drain
    click.confirm(f"Ending Series", default=True)
...
def measure(host, user, password, smu_ip, harvester, emulator):
    ...
    with Keithley2600(f"TCPIP0::{smu_ip}::INSTR") as kth, Connection(host, user=user, connect_kwargs=fabric_args) as cnx:
        ...
        print(f" -> Profiler will sweep through {len(currents_A)} currents {currents_A}")
