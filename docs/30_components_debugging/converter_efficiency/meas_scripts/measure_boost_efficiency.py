...
# CONFIG
vs_inputs = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.7, 0.8, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5]
vs_output = [2.0, 2.5, 3.0, 3.5, 4.0, ] # 4.5]
is_input = [
...
for vs_input in vs_inputs:
    ...
    for (v_out, v_inp) in product(vs_output, vs_input):
        ...
        print(f"Plotgroup (VOut={v_out}, VInp={v_inp}) has {countA} total entries, {countD} valid, {countE} medians")
        if len(data) <= 1:
            continue
...
results_processed.to_csv(path_here/"boost_results_processed.csv", sep=";", decimal=",", index=False)
