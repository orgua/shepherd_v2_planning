...
shp_cap1 = shp_cap_sim_boost(V_start_V=1e-3, P_inp_W=3 * 1e-3, runtime=runtime)
shp_cap2 = shp_cap_sim_current(V_start_V=0, I_inp_A=1e-3, V_max_V=3.0, runtime=runtime)
...
plt.savefig(Path(__file__).with_suffix(".svg"))  # can also output SVG
plt.close(fig)
